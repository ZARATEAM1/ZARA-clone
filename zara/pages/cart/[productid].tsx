...
export const getStaticProps: GetStaticProps = async (context) => {
    ...
    const res = await fetch(`http://localhost:3002/api/getone/${productid}`);
    const data = await res.json();
  
    return {
      props: { product: data, productid }
    };
  };
...
const Details: React.FC<{ product: Product; productid: string }> = ({ product, productid }) => {
